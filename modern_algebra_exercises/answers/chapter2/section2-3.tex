...
\zm{
	...
	$|G|=8$，其元素只能是$1,2,4,8$阶，且$8$阶元素$b$可推出$4$阶元素$b^2$存在. 若$G$包含$4$阶元$a$，则$\{1, a, a^2, a^3\}$的指数为$2$，它是$G$的非平凡正规子群，若$G$中非单位元全为$2$阶，由{\heiti 引理}\textbf{1.66}，$G$是阿贝尔群，任何$2$阶元素生成的子群都是非平凡正规子群.
...
\zm{
	...
	任取$gH\in G/H, (gH)^{|S_m|}=H$，即$g^{m!}\in H$.
	
	对于任意$a\in\mathbb{C}$，因为$\mathbb{C}$是代数封闭域，存在$b\in\mathbb{C}$使得$b^{m!}=a$，从而$(I-E_{ii}+bE_{ii})^{m!}=I-E_{ii}+aE_{ii}$，当$a\neq 0$时$b\neq 0$，前者属于$G$，故后者属于$H$.
	
	而$(I+a/m!\cdot E_{ij})^{m!}=I+aE_{ij}\:(i\neq j)$，前者属于$G$，故后者属于$H$.
...
\zm{
	...
	考虑$G$的左乘表示$\rho: G\hookrightarrow S_{2^km}$，则$\ker\rho=1$，$G\leq S_{2^km}$，令$H=G\cap A_{2^km}$，则$G/H\leq S/A, 
	...
	假设$P$是另一个$G_s$中的$m$阶子群. 由于$G_0\vartriangleleft G_s$，$PG_0=G_0P$，由{\heiti 习题}\textbf{1.2.16}知$PG_0$是$G_s$的子群，由{\heiti 定理}\textbf{1.70}知$|PG_0|=|P||G_0|/|P\cap G_0|=m^2/|P\cap G_0|$，故$|PG_0|$是奇数. 如果$|P\cap G_0|<m$，则$|PG_0|>m$，但$|PJ|$是$G_s$的因子，$G_s=2^sm$没有大于$m$的奇因数，矛盾，故$|P\cap G_0|=m$，即$P=G_0$.
...
\subsection{}
设$\alpha$是有限群$G$的自同构. 若$\alpha$把每个元素都变到它在$G$中的共轭元素，即对任意$g\in G$，$g$和$\alpha(g)$共轭，则$\alpha$的阶的素因子都是$|G|$的因子.
...
如果$G$为$n$阶有限群，$t$为$G$中共轭类的个数，$c=\frac{t}{n}$，证明$c=1$或$c\leq \frac{5}{8}$.
