...
\zm{
	...
	故$\sigma=\sigma_1\cdot(ij)$，其中$\sigma_1$为$\sigma$中$i$和$j$交换，$\sigma(i),\sigma(j)$不变而成，$\sigma_1$的题述集合和$\sigma$的题述集合相比，除了不含$(i,j)$，由引理三其余元素均不受影响. 故$n(\sigma_1)=n(\sigma)-1$，重复此过程直至$n(\sigma_{n(\sigma)})=0$则由题述集合的定义$\sigma_{n(\sigma)}$必为恒等置换，$\sigma$为$n(\sigma)$个对换和恒等置换之积.
}
