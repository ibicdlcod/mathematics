...
\zm{
	...
	$$\genfrac{(}{)}{0pt}{}{mp^r}{p^k}=|X|=\sum_{i=1}^{l}m+\sum_{j}p^{u_j}m\equiv lm\mod pm.$$
	
	故独立于$G$的结构我们有$\genfrac{(}{)}{0pt}{}{mp^r}{p^k}\equiv lm\mod pm.$，取$G_0=\mathbb{Z}/p^km\mathbb{Z}$得$l_{G_0}=1$，$|X|\equiv m\mod pm$，故$l\equiv 1\mod p$.
}
...
\zm{
	由{\heiti 定理}\textbf{2.5.3}，$G$中有$np+1\;(n\in\mathbb{N})$个$p$阶子群. 若这样的两个子群$P_1,P_2$满足$P_1\bigcap P_2\supsetneqq \{1\}$，则$1<|P_1\bigcap P_2| \mid |P_1|=p$，只能$|P_1|=|P_2|=|P_1\bigcap P_2|=p$，即$P_1=P_2$，故两个不同的$p$阶子群有且仅有$1$为公共元素.
...
\zm{
	若$p\nmid |N|$，$N$的西罗$p$-子群为平凡群，且$|P|$与$|N|$互素，由{\heiti 习题}\textbf{1.3.18}，$N\bigcap P$也是平凡群.
	
	若$p\mid |N|$，由西罗第二定理，存在$g$使得$P^{\prime}=gPg^{-1}$使得$P^{\prime}\bigcap N$是$N$的西罗$p$-子群，其共轭$g^{-1}(P^{\prime}\bigcap N)g=g^{-1}P^{\prime}g\bigcap g^{-1}Ng=P\bigcap N\subseteq N$也是$N$的西罗$p$-子群.
}
...
\zm{
	$|G|=p^rm, |N|=p^km_1, r\geq k, p\nmid m,m_1, m_1\mid m, |G/N|=p^{r-k}(m/m_1)$，$G/N$的西罗$p$-子群为$p^{r-k}$阶. 而（第二同构定理）$PN/N\cong P/P\bigcap N$，由(1)知$|P\bigcap N|=p^k$，$|P/P\bigcap N|=p^{r-k}$.
}
...
\zm{
	由第二同构定理，只需证$N_G(P)/N_G(P)\bigcap N\cong N_{G}(P/N\bigcap P)N$（右边是因映射$h\mapsto hN$诱导$PN/N$与$P/N\bigcap P$的同构）
	
	令$\varphi: N_G(P)=\{g\mid g^{-1}Pg=P, g\in G\}\rightarrow N_{G}(P/N\bigcap P)N=\{gN\mid g^{-1}P(N\bigcap P)g=P(N\bigcap P)\}=\{gN\mid g^{-1}Pg=P\}$，易见$\varphi$是满射，其核$\ker\varphi=\{n\mid n^{-1}Pn=P,n\in N\}=N_G(P)\bigcap N$，由第一同构定理即得结论.
}

\subsection{}
令$P_1,P_2,...,P_N$是有限群$G$的全部西罗$p$-子群，若对任意$i\neq j$总有
$$(P_i:P_i\bigcap P_j)\geq p^r,$$
则$N\equiv 1\mod p^r$.

\zm{
	由$P_i$是$p$-群，$(P_i:P_i\bigcap P_j)=p^{r+s}, s\geq 0$.
	...
	对于其他轨道$O_{P_j}$，考虑$N_G(P_j)\bigcap P_1$，它的阶整除$|P_1|$故是$p$-群，又是$N_G(P_j)$的子群，从而（{\heiti 推论}\textbf{2.51}）是$N_G(P_j)$的一个西罗$p$-子群的子群，但$P_j\vartriangleleft N_G(P_j)$为$N_G(P_j)$的唯一西罗$p$-子群，故$N_G(P_j)\bigcap P_1\leq P_j$是$P_1\bigcap P_j$的子群，$p^r\mid p^r+s=(P_1:P_1\bigcap P_j)\mid(P_1:N_G(P_j)\bigcap P_1)$，又$|O_{P_j}|=(P_1:N_{P_1}(P_j))=(P_1:P_1\bigcap N_G(P_j))$，故$|O_{P_j}|$被$p^r$整除.
...
\zm{
	...
	反证法，若只有一个这样的子群，则它是正规子群，记为$N_3\cong \mathbb{Z}/3\mathbb{Z}$. 考虑$G$的一个西罗$2$-子群$H_8$，我们有$|H_8|=8$. 由于$N_3H_8$的阶整除$24$且被$3$和$8$整除，故为$24$，即$N_3H_8=G$，类似地，$3$和$8$互素表明$N_3\bigcap H_8=\{1_G\}$.
