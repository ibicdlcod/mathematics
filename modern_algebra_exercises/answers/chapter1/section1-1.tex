...
\zm{
($\Rightarrow$)$f$为满射，则$\forall b\in B$， 存在原像$\{f^{-1}_{i}(b)\mid i \in I_b\}\neq \varnothing$(该集合不一定只有一个元素)

对所有的$b$选择$\{f^{-1}_{i}(b)\mid i \in I_b\}$中的一个元素$f^{-1}_1(b)$()这里用到了{\heiti 选择公理}，参见{\heiti 引理}\textbf{3.66}. 事实上(1)的证明中"A中任意元素"的表述也用到了选择公理)
...
\zm{
	构造单射$f:\mathbb{N}\rightarrow X$（$X$无限，因此这是可以做到的），其中$\{0,1,...,|Y|\}\mapsto Y$. 令
