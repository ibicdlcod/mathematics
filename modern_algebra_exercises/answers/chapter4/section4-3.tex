...
\zm{
	...
	对$f(x)$的任意一个根$u$，由$f(x)$不可约且不是一次多项式（$(x-\alpha)(x-\beta)\mid f(x)$，有$u\notin\mathbb{Q}$，定义同态$\varphi_u: \mathbb{Q}[x]\rightarrow\overline{\mathbb{Q}}, f(x)\mapsto f(u)$，则$(f(x))_{\Ideal, \mathbb{Q}[x]}\subseteq\ker\varphi_u$，由$\mathbb{Q}$是域，$\mathbb{Q}[x]$是PID，故也是贝祖环，当$g(x)\notin (f(x))_{\Ideal, \mathbb{Q}[x]}$时因$f(x)$不可约，$\gcd(f(x),g(x))=1, af(x)+bg(x)=1\;(a,b\in\mathbb{Q}[x]), bg(u)=af(u)+bg(u)=\varphi_u(1)=1$，故$u$不是$g(x)$的根，$\ker\varphi_u\subseteq(f(x))_{\Ideal, \mathbb{Q}[x]}$，故$\ker\varphi_u=(f(x))_{\Ideal, \mathbb{Q}[x]}$，$\mathbb{Q}[x]/f(x)=\mathbb{Q}[x]/\ker\varphi_u\cong\im\varphi_u=\mathbb{Q}[u]$.
